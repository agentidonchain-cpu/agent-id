 ...
 */
const signature_js_1 = require("./src/services/security/signature.js");
...
async function runTests() {
    ...
    const message = (0, signature_js_1.generateConfigMessage)({ identityHash, timestamp });
    console.log('Message:\n', message);
    console.log('✓ Config message generated\n');
    // Test 2: Sign Message
    console.log('Test 2: Sign Message');
    const { signature, walletAddress } = await (0, signature_js_1.signMessage)(message, TEST_PRIVATE_KEY);
    console.log('Wallet:', walletAddress);
    console.log('Signature:', signature.slice(0, 20) + '...');
    console.log('✓ Message signed\n');
    // Test 3: Verify Signature
    console.log('Test 3: Verify Signature');
    const verification = await (0, signature_js_1.verifySignature)(message, signature);
    ...
    const signatureData = await (0, signature_js_1.createConfigSignature)(identityHash, TEST_PRIVATE_KEY);
    console.log('Signature Data:', {
        walletAddress: signatureData.walletAddress,
        timestamp: signatureData.timestamp,
        signature: signatureData.signature.slice(0, 20) + '...',
    });
    const verifyResult = await (0, signature_js_1.verifyConfigSignature)(identityHash, signatureData.timestamp, signatureData.signature, signatureData.walletAddress);
    console.log('Verification Result:', verifyResult);
    console.log('✓ Full flow works\n');
    // Test 5: Attestation Message
    console.log('Test 5: Generate Attestation Message');
    const attestationMessage = (0, signature_js_1.generateAttestationMessage)({
